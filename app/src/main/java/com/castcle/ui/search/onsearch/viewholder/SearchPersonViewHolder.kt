...
import com.castcle.android.databinding.ItemSearchPersonBinding
...
//
//
//  Created by sklim on 8/10/2021 AD at 11:01.
...
class SearchPersonViewHolder(
    ...
    init {
        itemView.subscribeOnClick {
            click.invoke(
                SearchItemClick.PersonItemClick(
    ...
    override fun bindUiModel(uiModel: SearchUiModel) {
        ...
        if (uiModel is SearchUiModel.SearchFollowUiModel)
            with(binding) {
                ivAvatar.loadCircleImage(uiModel.avatar)
                tvDisplayName.text = uiModel.displayName
                tvTrendTopic.text = uiModel.displayName
            }
    }
    ...
    companion object {
        fun newInstance(
            ...
            val binding = ItemSearchPersonBinding.inflate(inflate, parent, false)
            return SearchPersonViewHolder(binding, clickItem)
        }
    }
}
