...
class FeedContentShortUserProfileViewHolder(
    val binding: LayoutFeedTemplateShortBinding,
    private val click: (Click) -> Unit
) : UserProfileAdapter.ViewHolder<ContentUiModel>(binding.root) {
    ...
    companion object {
        ...
        ): FeedContentShortUserProfileViewHolder {
            val inflate = LayoutInflater.from(parent.context)
            val binding = LayoutFeedTemplateShortBinding.inflate(inflate, parent, false)
            return FeedContentShortUserProfileViewHolder(binding, clickItem)
        }
    }
}
