...
class GetCachePageDataCompletableUseCase @Inject constructor(
    rxSchedulerProvider: RxSchedulerProvider,
    private val userPageDao: UserPageDao
) : SingleUseCase<Unit, PageHeaderUiModel>(
    rxSchedulerProvider.io(),
    rxSchedulerProvider.main(),
    ::Ignored
) {
    override fun create(input: Unit): Single<PageHeaderUiModel> {
        return userPageDao.getUserPage().map {
            it.toPageHeaderUiModel()
        }
    }
}
