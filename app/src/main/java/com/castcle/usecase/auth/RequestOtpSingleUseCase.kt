...
class RequestOtpSingleUseCase @Inject constructor(
    rxSchedulerProvider: RxSchedulerProvider,
    private val authenticationsRepository: AuthenticationsRepository
) : SingleUseCase<OtpRequest, VerificationUiModel>(
    rxSchedulerProvider.io(),
    rxSchedulerProvider.main(),
    CommonError::Error
) {
    override fun create(input: OtpRequest): Single<VerificationUiModel> {
        return authenticationsRepository.requestOtp(input)
    }
}
