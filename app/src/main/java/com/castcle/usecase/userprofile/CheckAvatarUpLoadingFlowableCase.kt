...
class CheckAvatarUpLoadingFlowableCase @Inject constructor(
    ...
    override fun create(input: Unit): Flowable<Pair<Boolean, String>> {
        return imageUploaderWorkHelper
            .uploadStatus
            .map {
                val status = if (it is ImageUploaderWorkHelper.Status.Error) {
                    throw AppError(cause = null, readableMessageRes = it.error)
                } else {
                    it is ImageUploaderWorkHelper.Status.Success
                }

                val profileImageUrl =
                    (it as? ImageUploaderWorkHelper.Status.Success)?.userResponse ?: ""
