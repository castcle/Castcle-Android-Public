...
sealed class UpLoadImageError(
    cause: Throwable?,
    @StringRes readableMessageRes: Int? = null
) : AppError(cause, (cause as? ApiException)?.error, readableMessageRes) {

    class UpLoadAvatarError(cause: Throwable?) :
        UpLoadImageError(cause, R.string.error_get_member)

    class UpLoadCoverError(cause: Throwable?) :
        UpLoadImageError(cause, R.string.error_get_member)
}
